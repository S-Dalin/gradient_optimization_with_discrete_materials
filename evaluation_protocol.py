...
def calculate_emd(real_data, synthetic_data):
    ...
    for i in range(real_data.shape[1]):
        if np.issubdtype(
            real_data[:, i].dtype, np.number
        ):  # Only calculate EMD for numerical columns
...
def calculate_js_divergence(real_data, synthetic_data, num_bins=10):
    ...
    for i in range(real_data.shape[1]):
        ...
        synthetic_counts, _ = np.histogram(
            synthetic_data[:, i], bins=num_bins, density=True
        )
...
def calculate_spca(X_real, X_synthetic, n_components=2, epsilon=1e-10):
    ...
    mape = (
        np.mean(
            np.abs(
                (log_explained_variance_real - log_explained_variance_synthetic)
                / log_explained_variance_real
            )
        )
        * 100
    )
