...
class WGAN_GP:
    def __init__(self, noise_dim, N_numerical, N_mat_core, N_mat_shell, g_lr, d_lr, beta_1, beta_2, n_critic, n_generator, gradient_penalty_weight):
        ...
        self.g_optimizer = Adam(learning_rate=self.g_lr, beta_1=self.beta_1, beta_2=self.beta_2, clipvalue=1.0)
        self.c_optimizer = Adam(learning_rate=self.d_lr, beta_1=self.beta_1, beta_2=self.beta_2, clipvalue=1.0)

        self.generator_loss = []
        self.critic_loss = []

    def build_generator(self):
        in_layer = tf.keras.Input(shape=(self.noise_dim,))

        x = tf.keras.layers.Dense(512, kernel_initializer='he_normal')(in_layer) #initialization is a method for initializing the weights of layers in a neural network. It is based on the work by He et al. (2015)
        ...
        out_1 = tf.keras.layers.Dense(self.N_numerical, activation='tanh', kernel_initializer='he_normal')(x)

        out_2 = tf.keras.layers.Dense(self.N_mat_core, kernel_initializer='he_normal')(x)
        out_2 = tf.keras.layers.Softmax()(out_2)

        out_3 = tf.keras.layers.Dense(self.N_mat_shell, kernel_initializer='he_normal')(x)
    ...
    def gradient_penalty(self, real, fake):
        epsilon = tf.random.uniform([real[0].shape[0], 1], minval=0.0, maxval=1.0, dtype=tf.dtypes.float32) # real[0].shape[0] = 122500 samples
        x_hat = [tf.cast(epsilon, tf.float32) * tf.cast(real_i, tf.float32) + (1 - tf.cast(epsilon, tf.float32)) * tf.cast(fake_i, tf.float32) for real_i, fake_i in zip(real, fake)]
        with tf.GradientTape() as t:
            t.watch(x_hat)
            d_hat = self.critic(x_hat)
        gradients = t.gradient(d_hat, x_hat)
        ddx = [tf.sqrt(tf.reduce_sum(gradients_i ** 2, axis=[1])+ 1e-10) for gradients_i in gradients]
        d_regularizer = tf.reduce_mean([(ddx_i - 1.0) ** 2 for ddx_i in ddx]) # compute L2 regulization
        return d_regularizer

    @tf.function
    def train_step(self, real_data):
        real_data = [tf.cast(real_data[..., :self.N_numerical], tf.float32), 
                     tf.cast(real_data[..., self.N_numerical:self.N_numerical + self.N_mat_core], tf.float32), 
        ...
        for _ in range(self.n_critic):
            with tf.GradientTape() as d_tape:
                noise = tf.random.normal([real_data[0].shape[0], self.noise_dim], dtype=tf.dtypes.float32)
            ...
            self.c_optimizer.apply_gradients(zip(d_gradients, self.critic.trainable_variables))
    ...
    def critic_loss_fn(self, real, fake):
        ...
        c_loss = tf.reduce_mean(logits_fake) - tf.reduce_mean(logits_real) + self.gradient_penalty_weight * gp
        return c_loss
    ...
    def plot_losses(self):
        ...
        with open('models/wgangp_tf/wgangp_loss_data.pkl', 'wb') as f:
...
def generate_synthetic_data(generator, noise_dim, num_samples):
    ...
    # synthetic_data = generator.predict(noise) #keras2
    synthetic_data = generator(noise) #keras3
    return synthetic_data


# Inverse transform synthetic data to the original scale
def inverse_transform_synthetic_data(preprocessor, synthetic_data, save_path='models/synthetic_data.npy'):
    """
    ...
    """
    # Concatenate the synthetic data
    synthetic_data = np.concatenate(synthetic_data, axis=1)

    # Split the synthetic data into numerical and categorical parts
    num_features_len = len(preprocessor.transformers_[0][2])  # Length of numerical features
    ...
    synthetic_data_original_scale = np.hstack((synthetic_data_num_original, synthetic_data_cat_original))
